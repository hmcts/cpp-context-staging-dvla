package uk.gov.moj.cpp.stagingdvla.aggregate;

import static org.hamcrest.CoreMatchers.notNullValue;
import static org.junit.jupiter.api.Assertions.assertDoesNotThrow;
import static uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.JsonPathAssertions.jsonPathAssertions;
import static uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.defendantAggregateScenario;
import static uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.scenarios;
import static uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.ExpectedEventsAssertion.expectedEventsJson;
import static uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.ExpectedEventsAssertion.noExpectedEvents;

import uk.gov.moj.cpp.stagingdvla.aggregate.DefendantAggregateTestSteps.Scenario;

import java.util.List;
import java.util.stream.Stream;

import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.Arguments;
import org.junit.jupiter.params.provider.MethodSource;

class DefendantAggregateReopenedAppScenariosTest {

    public static Stream<Arguments> testScenarios() {
        return scenarios(
                Arguments.of("DD-39832-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-39832-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac2/app-resulted.json",
                                        noExpectedEvents()
                                )
                ),
                Arguments.of("DD-39832-ac3",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac3/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac3/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac3/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac3/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98", "NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 3)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 3)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-39832-ac4a",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac4a/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac4a/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 3)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac4a/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac4a/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98", "NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 3)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 3)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-39832-ac5",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac5/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac5/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted with adj",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac5/app-resulted-adj.json",
                                        noExpectedEvents())
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac5/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac5/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98", "NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 3)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98"))))

                ),
                Arguments.of("DD-39832-ac6",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac6/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac6/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac6/app-resulted.json",
                                        noExpectedEvents())
                                .withNotifyDriverStep(
                                        "application hearing amended ",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac6/app-amended-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac6/app-amended-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-39832-ac7",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac7/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac7/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39832/ac7/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39832/ac7/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("oatsEndorsements", List.of("NE98", "NE98"))
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 3)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-40326-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40326/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40326/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40326/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40326/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)))
                ),
                Arguments.of("DD-40326-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40326/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40326/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40326/ac2/app-resulted.json",
                                        noExpectedEvents())
                ),
                Arguments.of("DD-40325-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac1/app-resulted.json",
                                        noExpectedEvents())
                ),
                Arguments.of("DD-40325-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac2/app-resulted.json",
                                        noExpectedEvents()
                                )
                ),
                Arguments.of("DD-40325-ac2b",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac2b/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac2b/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac2b/app-resulted.json",
                                        noExpectedEvents()
                                )
                ),
                Arguments.of("DD-40325-ac3",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac3/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac3/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac3/app-resulted.json",
                                        noExpectedEvents()
                                )
                ),
                Arguments.of("DD-40325-ac4",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "sjp case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac4/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac4/case-resulted-events.json")
                                )
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac4/app-resulted.json",
                                        noExpectedEvents()
                                )
                ),
                Arguments.of("DD-40325-ac5",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "sjp case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac5/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac5/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac5/referred-to-cc-resulted.json",
                                        noExpectedEvents()
                                )
                                .withNotifyDriverStep(
                                        "cc case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40325/ac5/reopen-created-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40325/ac5/reopen-created-resulted-events.json"))
                ),
                Arguments.of("DD-40358-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Remove")
                                                        .add("previous", notNullValue())))
                ),
                Arguments.of("DD-40358-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac2/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac2/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Remove")
                                                        .add("previous", notNullValue())))
                ),
                Arguments.of("DD-40358-ac3",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac3/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac3/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac3/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac3/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Remove")
                                                        .add("previous", notNullValue())))
                ),
                Arguments.of("DD-40358-ac4",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac4/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac4/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40358/ac4/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40358/ac4/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 1)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)))
                ),
                Arguments.of("DD-40328-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences[0].results.size()", 3)
                                                        .add("cases[0].defendantCaseOffences[0].results[2].label", "Conditional discharge")
                                                        .add("cases[0].defendantCaseOffences.size()", 1)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 1)
                                                        .add("updatedEndorsements", List.of("NE98"))))
                ),
                Arguments.of("DD-40328-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions().add("notificationType", "New")))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac2/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac2/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("previous", notNullValue())
                                                        .add("cases[0].defendantCaseOffences.size()", 3)
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98", "NE98"))))
                ),
                Arguments.of("DD-40328-ac3",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac3/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac3/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 1)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac3/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac3/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 1)))
                ),
                Arguments.of("DD-40328-ac4",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac4/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac4/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40328/ac4/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40328/ac4/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("NE98", "NE98"))))
                ),
                Arguments.of("DD-41665-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-41665/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-41665/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-41665/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-41665/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)))
                ),
                Arguments.of("DD-40359-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40359/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40359/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40359/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40359/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Remove")
                                                        .add("cases[0].defendantCaseOffences.size()", 0)
                                                        .add("previous", notNullValue())))
                ),
                Arguments.of("DD-40359-ac2",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40359/ac2/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40359/ac2/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40359/ac2/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40359/ac2/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 1)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)))
                ),
                Arguments.of("DD-40657-a1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40657/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40657/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 5)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40657/ac1/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40657/ac1/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 4)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 5)
                                                        .add("updatedEndorsements", List.of("CU30", "NE98", "NE98"))))
                                .withNotifyDriverStep(
                                        "application adjourned hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-40657/ac1/app-adj-hearing-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-40657/ac1/app-adj-hearing-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 4)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 4)
                                                        .add("updatedEndorsements", List.of("NE98", "NE98", "NE98", "CU30"))))
                ),
                Arguments.of("DD-39940-ac1",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 1)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1/app-resulted.json",
                                        noExpectedEvents())
                ),
                Arguments.of("DD-39940-ac1a",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1a/case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1a/case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "application hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1a/app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1a/app-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("previous", notNullValue())
                                                        .add("previous.cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("SP40"))))
                ),
                Arguments.of("DD-39940-ac1b",
                        defendantAggregateScenario()
                                .withNotifyDriverStep(
                                        "sjp case hearing resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1b/sjp-case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1b/sjp-case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "New")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)))
                                .withNotifyDriverStep(
                                        "sjp  case ref to cc",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1b/sjp-case-ref-to-cc-resulted.json",
                                        noExpectedEvents())
                                .withNotifyDriverStep(
                                        "sjp  case ref to cc and application created",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1b/sjp-case-ref-to-cc-app-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1b/sjp-case-ref-to-cc-app-resulted-events.json"))
                                .withNotifyDriverStep(
                                        "cc case resulted",
                                        "/testdata/drivernotifications/reopening/dd-39940/ac1b/cc-case-resulted.json",
                                        expectedEventsJson("/testdata/drivernotifications/reopening/dd-39940/ac1b/cc-case-resulted-events.json")
                                                .withAssertions(jsonPathAssertions()
                                                        .add("notificationType", "Update")
                                                        .add("cases[0].defendantCaseOffences.size()", 2)
                                                        .add("updatedEndorsements", List.of("SP40"))))
                )
        );
    }

    @ParameterizedTest(name = "{index} => {0}")
    @MethodSource("testScenarios")
    void shouldCreateDVLANotificationForReopeningApplication(final String name, final Scenario scenario) {
        assertDoesNotThrow(() -> scenario.run(name, new DefendantAggregate()));
    }
}
